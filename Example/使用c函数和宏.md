...
```objc
NSString * BRIDGE_FORMAT_JS(void);
```
...
```objc
#define __jjrjsbridge_func__(x) #x

NSString * BRIDGE_FORMAT_JS() {

    static NSString * const rrjscode = @__jjrjsbridge_func__(
(!function () {
    if (window.JSBridge) {
        return;
    }
